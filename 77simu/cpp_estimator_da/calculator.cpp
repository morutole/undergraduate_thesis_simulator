...
Vector3d velocity_differential(const Vector3d& position, const Vector3d& velocity, const Vector3d& acceleration);

Matrix9d update_M_matrix(Matrix9d M, const Vector3d position, const Vector3d velocity, const Vector3d acceleration);
Matrix9d calculate_A_matrix(const Vector3d position, const Vector3d velocity, const Vector3d acceleration);
Matrix9d calculate_Q_matrix();
...
void Runge_kutta(vector<Vector3d, aligned_allocator<Vector3d>>& position_estimate, vector<Vector3d, aligned_allocator<Vector3d>>& velocity_estimate, vector<Vector3d, aligned_allocator<Vector3d>>& acceleration_estimate, Matrix9d &M, vector<Vector9d, aligned_allocator<Vector9d>>& M_store_vector)
{
    ...
    for(i = 0;i < iteration_number;++i){
        ...
        Vector3d l0 = velocity_differential(position, velocity, accelration);
        ...
        M = update_M_matrix(M, position, velocity, accelration);
...
void Kalman_Filter(vector<Vector3d, aligned_allocator<Vector3d>>& position_estimate, vector<Vector3d, aligned_allocator<Vector3d>>& velocity_estimate, vector<Vector3d, aligned_allocator<Vector3d>>& acceleration_estimate, Matrix9d &M, vector<Vector9d, aligned_allocator<Vector9d>>& M_store_vector, const Vector3d true_position, const Vector3d true_velocity, vector<Vector3d, aligned_allocator<Vector3d>>& position_observed, vector<Vector3d, aligned_allocator<Vector3d>>& velocity_observed, vector<Vector9d, aligned_allocator<Vector9d>>& estimate_error)
{
	...
	Matrix<double, 6, 9> H = MatrixXd::Zero(6, 9); //観測行列
	for(i = 0;i < 6;++i){
		H(i,i) = 1.0;
	}

	Matrix<double, 9, 6> K; //カルマンゲイン
	...
	Vector9d x_predict; //事前推定値
	x_predict << estimate_position(0), estimate_position(1), estimate_position(2), estimate_velocity(0), estimate_velocity(1), estimate_velocity(2), estimate_acceleration(0), estimate_acceleration(1), estimate_acceleration(2);

	Vector9d x_update = x_predict + K*(z_vector - H*x_predict); //事後推定値
	M = (MatrixXd::Identity(9, 9) - K*H)*M;
	...
	Vector9d x_error = K*(z_vector - H*x_predict);
...
Matrix9d update_M_matrix(Matrix9d M, const Vector3d position, const Vector3d velocity, const Vector3d acceleration)
{
    Matrix9d A = calculate_A_matrix(position, velocity, acceleration); //Jacobi行列
	Matrix9d Phi = MatrixXd::Identity(9,9) + diff_t * A;

	Matrix9d Q = calculate_Q_matrix();
